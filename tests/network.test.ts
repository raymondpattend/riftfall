...
it('heartbeats without rendering and closes stale snapshots even if pongs arrive',async()=>{
 ...
 expect(s.sent.filter(p=>p.type==='ping')).toHaveLength(5);expect(h.leave).not.toHaveBeenCalled();await vi.advanceTimersByTimeAsync(2000);expect(h.leave).toHaveBeenCalledOnce();expect(s.readyState).toBe(3);expect(vi.getTimerCount()).toBe(0);
