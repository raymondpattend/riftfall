...
try{
 ...
 await collect('healing_totem');await host.evaluate(()=>{const e=window.__riftfall;e.body.z=28;Object.assign(e.sim.players.get('host'),{z:28,hp:40});});await guest.keyboard.press('KeyX');await host.waitForFunction(()=>window.__riftfall.player.hp>=45);await guest.waitForFunction(()=>window.__riftfall.snapshot.fields.some(f=>f.kind==='healing_totem'));report('PASS shared healing field heals the opposing player');
